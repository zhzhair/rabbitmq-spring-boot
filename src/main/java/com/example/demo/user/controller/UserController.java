...
public class UserController extends BaseController {
    ...
    @ApiOperation(value = "添加测试用户数据", notes = "添加测试用户数据，异步添加登录历史记录")
    ...
    @ApiOperation(value = "添加测试用户数据", notes = "添加测试用户数据，异步添加登录历史记录")
    @RequestMapping(value = "/loginForTest", method = {RequestMethod.GET}, produces = {MediaType.APPLICATION_JSON_VALUE})
    public BaseResponse<Object> loginForTest() {
        BaseResponse<Object> baseResponse = new BaseResponse<>();
        String[] userNames = {"xiaoming","xiaohong","jingjing","xiaoqiang"};
        String password = "123456";
        int rand = new Random().nextInt(userNames.length);
        if (userService.login(userNames[rand], password)) {
            baseResponse.setCode(0);
            baseResponse.setMsg("登录成功");
        } else {
            baseResponse.setCode(1);
